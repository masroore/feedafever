...
class OMDOMDOM
{
    public function parse($xml)
    {
        ...
        if ('fffe' == bin2hex(substr($xml, 0, 2))) {
            $encoding = 'UTF-16';
        }
        ...
        $nodes =  // just node arrays by node id
        [
            0 => [
                '_node_id' => 0,
                '_node_name' => '#document',
        ...
        $ancestors =  // flat associative array, of tagnames indexed by node id
        [
            0 => '#document',
        ];
        ...
        foreach ($tag_fragments as $tag_fragment) {
            ...
            [$tag, $content] = array_pad(array_slice($n, 1, 2), 2, '');
            ...
            // handle opening tags
            [$tag_name, $attrs_str] = array_pad(preg_split('/\s+/', $tag, 2), 2, '');
...
class OMDOMDOMNode
{
    ...
    public function get_descendant_nodes($__children_only = false, $__node_id_only = false)
    {
        ...
        foreach ($__OMDOMDOM_nodes as $node_id => $COPY_node) {
            if (
                0 === strpos($COPY_node['_node_ancestry'], $ancestry) &&
                (!$__children_only || $COPY_node['_node_ancestry'] == $parent)
            ) {
                $nodes[] = $__node_id_only ? $node_id : new self($node_id);
            }
        }
    ...
    public function get_nodes_by($attr, $value = '', $__children_only = false)
    {
        ...
        foreach ($nodes as $node_id) {
            ...
            if (isset($node[$attr]) && $node[$attr] == $value) {
                $matching_nodes[] = new self($node_id);
            }
        }
    ...
    public function inner_content()
    {
        ...
        if ('#' == $node['_node_name'][0] && isset($node['_node_content'])) {
            ...
            if ('#cdata' == $node['_node_name']) {
                $cdata = r('#<!\[CDATA\[(.*?)]]>#smu', '$1', $node['_node_content']);
                $parent = $this->parent();
                if ('content:encoded' == $parent->get_node_name()) {
