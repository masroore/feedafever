...
/******************************************************************************
 ...
 REQUEST
 ******************************************************************************/
if (!defined('REQUEST_UA')) {
    define('REQUEST_UA', 'SI_Request');
}
if (!defined('REQUEST_REDIRECT_LIMIT')) {
    define('REQUEST_REDIRECT_LIMIT', 4);
}
if (!isset($REQUEST_TIMEOUT)) {
    $REQUEST_TIMEOUT = 10;
} // seconds
if (!isset($REQUEST_SOCKET)) {
    $REQUEST_SOCKET = false;
}
...
function request($method = '', $url = '', $post = '', $headers = [], $following_redirect = false)
{
    ...
    debug(['timeout' => $time_out, 'url' => $parsed_url, 'headers' => $headers], 'request ('.($use_curl ? 'curl' : 'socket').')');
    ...
    foreach ($response[0] as $header) {
        [$key, $value] = filled_explode(': ', $header, 2);
        ...
        if ('Status' == $key && empty($response_headers['response_code'])) {
            if (m('#[0-9]{3}#', $value, $m)) {
    ...
    if (isset($response_obj['headers']['set-cookie'])) {
        foreach ($response_obj['headers']['set-cookie'] as $set_cookie) {
            ...
            if (isset($cookie_parts[0]) && !empty($cookie_parts[0])) {
                [$cookie_name, $cookie_value] = explode('=', $cookie_parts[0], 2);
