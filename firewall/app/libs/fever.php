...
class fever
{
    ...
    public $total_feeds = 0;
    public $total_items = 0;
    public $total_unread = 0;
    public $total_saved = 0;

    public $last_refreshed_on_time = 0;
    public $last_cached_on_time = 0;

    public $links_by_degrees = [];
    public $group_ids_by_feed_id = [];
    public $feed_ids_by_group_id = [];
    public $sparks_feed_ids = [];
    public $saved_feed_ids = [];
    public $feed_ids = [];
    ...
    public function __construct()
    {
        ...
        if (err()) {
            debug(['GET' => $_GET, 'POST' => $_POST, 'capabilities' => $this->capabilities()], 'New instance of Fever');
        }
    }
    ...
    public function capabilities()
    {
        ...
        $caps['php_version'] = version_clean(PHP_VERSION);
        ...
        if ($caps['has_curl'] && in(ini_get('disable_functions'), 'curl_')) {
            $caps['has_curl'] = 0;
        }
    ...
    public function drop_error($containing = '')
    {
        if (empty($containing)) {
            ...
            foreach ($this->errors['list'] as $error) {
                if (false !== strpos($error, $containing)) {
                    continue;
                }
                $errors_list[] = $error;
            }
    ...
    public function connect()
    {
        ...
        if ($this->dbc->error) {
            $this->fatal_error($this->dbc->error);
        }
        if (!$connected) {
            $this->annotate_error('<p>Fever was unable to connect to the database.</p>');
        }
    ...
    public function save_one($table, $row_data, $where = '')
    {
        ...
        if (isset($row_data['id'])) { // update
            ...
            foreach ($row_data as $col => $value) {
                if ('id' == $col || false === strpos($this->manifest[$table], "`{$col}`")) {
                    continue;
                }
            ...
            $values[] = $row_data['id'];
            $update = call_user_func_array([$this, 'prepare_sql'], $values);
            $this->query($update);
        } else { // insert
            $columns = [];
            $values = [];
            foreach ($row_data as $col => $value) {
                if (false === strpos($this->manifest[$table], "`{$col}`")) {
                    continue;
                }
                $columns[] = $col;
                $values[] = $value;
            ...
            if (!empty($columns)) {
                ...
                array_unshift($values, $sql);
                $insert = call_user_func_array([$this, 'prepare_sql'], $values);
    ...
    public function prepare_sql($query)
    {
        ...
        return call_user_func_array([$this->dbc, 'prepare'], $args);
    }
    ...
    public function route_api()
    {
        ...
        if (isset($_GET['groups']) || isset($_GET['feeds'])) {
            ...
            foreach ($feeds as $feed) {
                $data_feeds[] = [
                    'id' => (int) ($feed['id']),
                    'favicon_id' => $feed['favicon_id'] ? (int) ($feed['favicon_id']) : 1,
                    'title' => $this->title($feed),
                    'url' => $feed['url'],
                    'site_url' => $feed['site_url'],
                    'is_spark' => $feed['is_spark'] ? 1 : 0,
                    'last_updated_on_time' => (int) ($feed['last_updated_on_time']),
                ];

                if ($feed['is_spark']) {
                    $spark_feeds[] = (int) ($feed['id']);
                }
            }
            unset($feeds);
            ...
            foreach ($feeds_to_groups as $feed_group) {
                if (in_array($feed_group['feed_id'], $spark_feeds)) {
                    continue;
                } // ignore spark feeds
                $feeds_by_group[$feed_group['group_id']][] = (int) ($feed_group['feed_id']);
            }
        ...
        if (isset($_GET['items'])) {
            ...
            if (isset($_GET['feed_ids']) || isset($_GET['group_ids'])) { // added 0.3
                ...
                if (isset($_GET['group_ids'])) {
                    ...
                    $args = array_merge([$query], $group_ids);
                    $feed_ids_where = call_user_func_array([$this, 'prepare_sql'], $args);
                ...
                $args = array_merge([$query], $feed_ids);
                $where .= call_user_func_array([$this, 'prepare_sql'], $args);
            }
            ...
            if (isset($_GET['max_id'])) { // descending from most recently added
            ...
            } elseif (isset($_GET['with_ids'])) { // selective
                if (!empty($where)) {
                    $where .= ' AND ';
                ...
                $args = array_merge([$query], $item_ids);
            ...
            foreach ($items as $item) {
                $data['items'][] = [
                    ...
                    'created_on_time' => (int) ($item['created_on_time']),
                ];
            }
            unset($items);
        }
        ...
        if (isset($_GET['links'])) {
            $data['links'] = [];
            // set temporarily
            $this->prefs['ui']['hot_start'] = isset($_GET['offset']) ? (int) ($_GET['offset']) : 0; // now
            $this->prefs['ui']['hot_range'] = isset($_GET['range']) ? (int) ($_GET['range']) : 7; // week
    ...
    // render_api()
    public function render_api($data = [], $output = '')
    {
    ...
    public function route_image()
    {
        ...
        if (200 == $response['headers']['response_code']) {
    ...
    public function route_manage()
    {
        if (isset($_POST['action'])) {
            switch ($_POST['action']) {
                ...
                case 'import':
                    if (isset($_FILES['opml']) && UPLOAD_ERR_OK == $_FILES['opml']['error']) {
                ...
                case 'export':
                    $group_ids = ($_POST['with_groups']) ? $_POST['group_ids'] : [];
                ...
                case 'edit-sparks':
                    $feed_ids = (!isset($_POST['feed_ids'])) ? [] : $_POST['feed_ids'];
    ...
    {
        if (isset($_GET['url'])) {
            if ($feed_id = $this->add_feed(['url' => $_GET['url']])) {
    ...
    public function capture_state($sender = 'ui')
    {
        if (isset($_GET[$sender])) {
            if (isset($_GET[$sender]['section']) && 4 != $this->prefs['ui']['section'] && FEVER_MOBILE != $sender) {
...
PHP;
    ...
    public function is_development_copy()
    {
        ...
        return ('localhost' == $paths['trim'] || 'scratch.dev' == $paths['trim']) ? true : false;
    }
    ...
    public function update_files()
    {
        if (
            ...
            if (!empty($response['error']['msg'])) {
                ...
                $this->render('errors');
                $this->close();
            } elseif (!isset($response['headers']['X-Apptivator-Verified'])) {
                $this->error('Invalid response headers from the gateway ('.prevent_xss(array_to_query($response['headers'])).')');
                $this->render('errors');
                $this->close();
            } elseif (!$response['headers']['X-Apptivator-Verified']) {
    ...
    {
        $paths = $this->install_paths();
        $response = post('http://feedafever.com/gateway/', [
            'app_name' => low($this->app_name),
            'activation_key' => ACTIVATION_KEY,
            'domain_name' => $paths['trim'],
            'capabilities' => $this->capabilities(),
        ],
            ['X-Apptivator-Action:'.$action]);
    ...
    public function update()
    {
        ...
        if ($this->cfg['version'] < 105) {
            ...
            $i = 2;
            while (false !== $this->dbc->query("ALTER TABLE `{$this->db['prefix']}links` DROP INDEX `is_first_{$i}`")) {
    ...
    public function install_paths()
    {
        ...
        $protocol = ((isset($_SERVER['HTTPS']) && 'on' == low($_SERVER['HTTPS'])) || (isset($_SERVER['HTTP_HTTPS']) && 'on' == low($_SERVER['HTTP_HTTPS'])) || (isset($_SERVER['SERVER_PORT']) && 443 == $_SERVER['SERVER_PORT'])) ? 'https' : 'http';
    ...
    public function load()
    {
        if ($data = $this->get_one('_config')) {
            ...
            $unserialize_func = 0 === strpos($data['cfg'], 'a:') ? 'unserialize' : 'unserialize_safe';
    ...
    public function import($opml, $import_groups = true)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if ($has_text && !$is_feed) { // group
                ...
                $is_spark = ('Sparks' == $group['title']);

                if ($import_groups && 'All' != $group['title'] && 'Sparks' != $group['title']) {
                    $group_id = $this->add_group($group);
                }
            } elseif ($is_feed) { // feed
                ...
                if ('body' == $parent_node_name && $import_groups) {
                    $group_id = 0;
                }
    ...
    // export()
    public function export($group_ids = [], $flatten = false, $include_sparks = false)
    {
        $this->relationships();
        $title = 'Fever';
        $file_name = 'fever';
        ...
        if (1 == count($group_ids) && 0 != $group_ids[0]) {
        ...
        foreach ($group_ids as $group_id) {
            ...
            foreach ($feed_ids as $feed_id) {
                if (0 == $feed_id || ($flatten && in($output_feed_ids, $feed_id))) {
                    continue;
                }
    ...
    public function add_group($group = [])
    {
        ...
        if (!empty($group['title'])) {
            ...
            elseif ($existing = $this->get_one('groups', $this->prepare_sql('`title` = ?', $group['title']))) {
    ...
    // edit_feed()
    public function edit_feed($feed = [])
    {
        if (!isset($feed['id'])) {
            return;
        }
    ...
    public function add_default_favicon()
    {
        ...
        return $this->add_favicon(['url' => 'favicon.png', 'cache' => $default_favicon]);
    }
        ...
        foreach ($objs as $obj) {
            ...
            if (elapsed_execution_time() + ($average_execution_time * 1.5) < $available_execution_time) {
                ...
                if (method_exists($this, $each_method)) {
                    call_user_func_array([$this, $each_method], [$obj, $i, $total]);
    ...
    public function housekeeping()
    {
        ...
        if ($do_optimize) {
            $this->cfg['last_optimize_on_time'] = time();
        }
        if ($do_repair) {
            $this->cfg['last_repair_on_time'] = time();
        }

        if ($do_optimize || $do_repair) {
            $this->save();

            $tables = ['items', 'links', 'feeds'];
            foreach ($tables as $table) {
                ...
                if ($do_repair) {
                    ...
                    if ($rows = $this->query_all("CHECK TABLE {$this->db['prefix']}{$table} FAST")) {
                        ...
                        $status = end($rows);
                        if ('error' == $status['Msg_type']) {
                            $this->query("REPAIR TABLE {$this->db['prefix']}{$table}");
                        }
                    }
                }
            }
...
HTML;
    ...
    public function refresh_feed($feed_or_id)
    {
        ...
        if (0 == $this->get_count('links', $this->prepare_sql('`feed_id` = ? AND `is_first` = 1', $feed['id']))) {
            $reweight = true;
        }
        ...
        if (401 == $request['headers']['response_code']) {
        ...
        if (
            404 == $request['headers']['response_code'] ||  // missing
        ...
        if (empty($feed['site_url'])) {
            ...
            foreach ($links as $link) {
                ...
                if ('feed' == $parent_node_name) {
                    ...
                    if ($has_href && (!$has_rel || ($has_rel && 'self' != $rel && 'hub' != $rel && 'license' != $rel && !m('#^http#', $rel, $r)))) {
                ...
                } elseif ('channel' == $parent_node_name) {
        ...
        foreach ($items as $i => $item_node) {
            ...
            if ($existing = $this->get_one('items', $this->prepare_sql('`feed_id` = ? AND `uid` = ?', $feed['id'], $new_item['uid']))) {
                ...
                foreach ($new_item as $key => $value) {
                    if ('created_on_time' == $key || 'added_on_time' == $key) {
                        continue;
                    }
                ...
                if (!empty($update)) {
                    ...
                    $update[] = $existing['id'];
                    $query = call_user_func_array([$this, 'prepare_sql'], $update);
                    $this->query($query);
                ...
                $query = call_user_func_array([$this, 'prepare_sql'], $vals);
...
HTML;
    ...
    public function cache_favicon($feed)
    {
        ...
        foreach ($favicon_urls as $favicon_url) {
            ...
            if (
                ...
                404 != $request['headers']['response_code'] &&        // not a 404
        ...
        if (isset($favicon_data)) {
            ...
            if (
                ('text/x-icon' == $favicon_type || 'image/x-icon' == $favicon_type || 'image/vnd.microsoft.icon' == $favicon_type) && // ico
    ...
    public function parse_item($item_node, $feed)
    {
        ...
        foreach ($child_nodes as $node) {
            switch (low($node->get_node_name())) {
                ...
                case 'summary': // atom
                    ...
                    if (!empty($item['description']) && ('summary' == $node_name || 'description' == $node_name)) {
                        continue;
                    }
                ...
                case 'link':
                    if (empty($item['link'])) {
                        if ($node->has_attr('href')) { // atom
                            if (!$node->has_attr('rel') || 'alternate' == $node->get_attr('rel')) {
                ...
                case 'id': // atom
                    ...
                    if ($node->has_attr('isPermaLink') && 'false' == $node->get_attr('isPermaLink')) {
        ...
        if (false !== strpos($item['link'], '/rd?') && m('#&(rd=http[^&]+)#i', $item['link'], $m)) {
    ...
    public function build_blacklist_regexp()
    {
        ...
        if (!isset($blacklist_regexp)) {
            ...
            foreach ($blacklist_arr as $blacklisted) {
                ...
                if (0 === strpos($blacklisted, '#')) { // regular expression
    ...
    // mark_*_as_*()
    public function mark_items_as_read($item_ids = [])
    {
        if (!empty($item_ids)) {
            ...
            $unprepared_array = [$unprepared_query, time()];
            $unprepared_array = array_merge($unprepared_array, $item_ids);
            $update = call_user_func_array([$this, 'prepare_sql'], $unprepared_array);
            $this->query($update);
        }
    }

    public function mark_items_as_unread($item_ids = [])
    {
        if (!empty($item_ids)) {
            $where_in = '`id` IN (?'.str_repeat(',?', count($item_ids) - 1).')';
            $unprepared_query = "UPDATE `{$this->db['prefix']}items` SET `read_on_time` = 0 WHERE {$where_in}";
            $unprepared_array = [$unprepared_query];
            $unprepared_array = array_merge($unprepared_array, $item_ids);
            $update = call_user_func_array([$this, 'prepare_sql'], $unprepared_array);
            $this->query($update);
        }
    ...
    {
        $this->mark_items_as_read([$item_id]);
    }

    public function mark_item_as_unread($item_id)
    {
        $this->mark_items_as_unread([$item_id]);
    }
    ...
    public function mark_feeds_as_sparks($feed_ids = [])
    {
        ...
        $unprepared_array = array_merge([$unprepared_query], $feed_ids);
        $update = call_user_func_array([$this, 'prepare_sql'], $unprepared_array);
        $this->query($update);
    }
    ...
    public function highlight($str)
    {
        if (4 == $this->prefs['ui']['section']) {
            ...
            $str = r('#('.preg_quote($q, null).')#i', '<span class="q">\1</span>', $str); // highlight
...
HTML;
    ...
    public function relationships($rebuild = false)
    {
        if (empty($this->groups) || $rebuild) {
            ...
            $saved_feed_ids = array_merge([0], array_keys($saved_by_feed));
            $total_saved = 0;

            $feeds = $this->get_all('feeds', '1 ORDER BY `title` ASC, `url` ASC');

            $items = 'items';
            if (!$this->prefs['ui']['show_read'] && 4 != $this->prefs['ui']['section']) {
                $items = 'unread';
            }

            if (2 == $this->prefs['ui']['section']) {
                $items = 'saved';
            }
            ...
            switch ($this->prefs['ui']['section']) {
                case 4: // search
                    if (!empty($this->prefs['ui']['search'])) {
                        ...
                        $items_cols = ['title', 'link', 'description'];
                        ...
                        $this->feed_ids = array_unique(array_merge([0], $feed_ids, $item_feed_ids));
            ...
            $this->feed_ids = array_merge([0], array_values($feed_ids_by_title));
            ...
            if (!$this->prefs['ui']['show_read'] && 1 == $this->page) {
                if (1 == $this->prefs['ui']['section']) {
                    ...
                    if (isset($this->feeds[$focused_feed_id])) {
                        ...
                        if (0 == $focused_feed['unread_count']) {
    ...
    public function build_links()
    {
        ...
        $url_checksums = [];
        $hot_checksums = []; // tmp array
        foreach ($url_checksums_by_weight as $checksum) {
            if ($checksum['weight'] > $scale) {
                $url_checksums[] = $checksum['url_checksum'];
                $hot_checksums[] = $checksum;
            }
        }
        ...
        $link_ids_by_url_checksum = [];
        // get related item ids to retrieve items
        $item_ids = [];
        foreach ($links as $link) {
            $item_ids[] = $link['item_id'];
            if (!isset($link_ids_by_url_checksum[$link['url_checksum']])) {
                $link_ids_by_url_checksum[$link['url_checksum']] = [];
            }
            $link_ids_by_url_checksum[$link['url_checksum']][] = $link['id'];
        }
        ...
        foreach ($url_checksums_by_weight as $checksum) {
            ...
            $link_titles = []; // a/link title
            $source_ids = []; // ids of the items that contain this url
            ...
            foreach ($link_ids as $link_id) {
                ...
                if (isset($items[$link['item_id']])) {
                    ...
                    if (strlen(html_entity_decode_utf8($link['title'])) > 1 && '[link]' != $link['title']) {
    ...
    public function build_items()
    {
        ...
        if (
            (1 == $this->prefs['ui']['section'] || 3 == $this->prefs['ui']['section']) &&
